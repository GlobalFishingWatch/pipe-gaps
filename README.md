...
Time gap detector for vessel position reporting systems (**[AIS]**, **[VMS]**, etc).
...
[messages.py]: src/pipe_gaps/queries/messages.py
...
Not all vessel position reports generated by onboard reporting systems (such as **AIS** or **VMS**)
are successfully recorded or received.
For **AIS**, this may occur due to signal interference in crowded waters,
reception limitations,
and variability in terrestrial or satellite coverage,
and dropped signals as vessels move from terrestrial coverage
to areas of poor satellite reception.
For **VMS** and other satellite-based reporting systems,
gaps may result from device malfunction,
loss of power,
antenna damage,
or store-and-forward behavior during connectivity outages.
As a result,
it is not uncommon to observe gaps in vessel position reporting data
lasting hours or even days. [[1]](#1). 

Other causes of reporting gaps include onboard equipment being intentionally turned off,
malfunctioning hardware, or vessel systems being powered down while at anchor or in port.

Detecting reporting gaps is essential to identify
potential *intentional disabling events*,
which may obscure illegal activities
such as unauthorized fishing or transshipment [[1]](#1)[[2]](#2).
...
We create an **gap** event when the period of time between
...
The gap detection pipeline fetches vessels position messages,
...
Gaps are generated including vessels position messages
...
The gap detection core process takes as input a list of vessels positions messages.
...
```shell
(.venv) $ pipe-gaps -h
usage: pipe-gaps (v0.9.1). [-h] <command> ...
...
1. **Groups the main inputs** — **messages** — by `SSVID` and time intervals (`TI`) using sliding windows.  
2. **Groups side inputs** — open gaps — by `SSVID`.
3. **Extracts boundaries** — the first and last *N* messages of each group of main inputs.
...
| Module | Description |
| --- | --- |
| [gaps.py]         | Encapsulates gaps query. |
| [messages.py]     | Encapsulates position messages query. |
| [detect_gaps.py]  | Defines **DetectGaps** [PTransform] (core processing step of the pipeline). |
| [gap_detector.py] | Defines lower level **GapDetector** class that computes gaps in a list of position messages. |
...
```mermaid
flowchart TD;
    A[Read Main Inputs] ==> |**Messages**| B[Group Inputs]
    C[Read Side Inputs] ==> |**Open Gaps**| D[Group Side Inputs]

    subgraph **DetectGaps** PTransform
    B ==> |**Messages <br/> by SSVID & TI**| E[Process Groups]
    B ==> |**Messages <br/> by SSVID & TI**| F[Process Boundaries]
