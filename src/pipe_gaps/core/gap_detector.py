...
class GapDetector:
    ...
    @classmethod
    def message_sort_key(cls, m: dict) -> tuple:
        # Primary key is timestamp. Secondary keys deterministically break
        # ties when multiple messages share the same timestamp for the same
        # ssvid -- a known VMS data-quality phenomenon driven by concurrent
        # segments. Without these tiebreakers, OFF/ON candidates picked at
        # boundary handoffs depend on upstream order (Beam multimap
        # iteration, BQ row order, etc.), and gap output (gap_id encodes
        # the OFF's lat/lon; end_* columns reflect the picked ON) differs
        # across pipeline modes -- breaking mode-equivalence.
        #
        # Must be applied consistently anywhere messages are sorted, min'd,
        # or binary-searched at a partition / window / range boundary.
        return (
            m[cls.KEY_TIMESTAMP],
            m.get(cls.KEY_SEG_ID) or "",
            m.get(cls.KEY_MSGID) or "",
            m.get(cls.KEY_LAT) or 0.0,
            m.get(cls.KEY_LON) or 0.0,
        )

    # @profile  # noqa  # Uncomment to run memory profiler
    def _sort_messages(self, messages: list) -> None:
        messages.sort(key=self.message_sort_key)
