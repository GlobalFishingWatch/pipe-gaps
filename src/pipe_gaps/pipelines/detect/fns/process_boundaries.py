...
class Boundaries:
    ...
    def __init__(self, boundaries: list[Boundary]) -> None:
        self._boundaries = sorted(
            boundaries, key=lambda x: GapDetector.message_sort_key(x.first_message())
        )
    ...
    def get_first_message_inside_range(self, date_range: tuple = None) -> dict:
        ...
        the chronologically earliest message at or after date_range[0]. Ties on
        timestamp are broken by GapDetector's canonical key so the picked ON
        message is deterministic across pipeline modes.
        ...
        return min(candidates, key=GapDetector.message_sort_key, default=None)
